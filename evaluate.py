...
class CaptionEvaluator:
    ...
    def _evaluate(self, client_payload, _context={}):
        ...
        #cider = self.compute_cider(predictions)
        ...
        #assert "score_cider" in _result_object
    ...
    def load_gt(self):
        ...
        with open(self.ground_truth_path) as csvfile:
            reader = csv.reader(csvfile, delimiter='|', quoting=csv.QUOTE_NONE)
            next(reader) # if there is no header --> comment out
            for i, row in enumerate(reader):
                # if i>=2000 and i<=2999:
                #     pairs[row[0]]= row[1] //uncomment creates imbalanced data 
                pairs[row[0]] = row[1]
        return pairs
    ...
    def load_predictions(self, submission_file_path):
        ...
        with open(submission_file_path) as csvfile:
            ...
            lineCnt = 0
            occured_images = []
            next(reader) # if there is no header --> uncomment
            for row in reader:
                # print(row)
                lineCnt += 1
...
if __name__ == "__main__":
    ...
    ground_truth_path = "/home/msamprovalaki/captioning/results/test_gold_captions.csv"
    submission_file_path='/home/msamprovalaki/captioning/results/test_predicted_captions.csv'
